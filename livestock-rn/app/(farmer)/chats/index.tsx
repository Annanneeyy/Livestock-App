...
export default function ChatListScreen() {
  ...
  const renderChatItem = ({ item }: { item: any }) => {
    ...
    return (
      <TouchableOpacity
        onPress={() => router.push(`/(farmer)/chats/${item.id}`)}
      ...
      >
        ...
        <View className="flex-1 ml-4">
          ...
          <View className="flex-row justify-between items-center mt-1">
            <Text 
              className={`flex-1 text-sm ${item.unread_count > 0 ? 'text-gray-900 font-semibold' : 'text-gray-500'}`} 
              numberOfLines={1}
            >
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View className="flex-1 justify-center items-center mt-20 p-8">
            ...
            <Text className="text-gray-900 font-bold text-lg">No Messages Yet</Text>
