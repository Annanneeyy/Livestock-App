    ...
    try {
      if (isEditing && params.editId) {
        ...
        if (Platform.OS === 'web') {
          ...
          router.replace(`/${rolePath}/marketplace/${params.editId}`);
        } else {
          Alert.alert('Success', 'Listing updated!', [
            { text: 'OK', onPress: () => router.replace(`/${rolePath}/marketplace/${params.editId}`) },
          ]);
        }
      } else {
        ...
        if (Platform.OS === 'web') {
          ...
          router.replace(`/${rolePath}/marketplace/${newId}`);
        } else {
          Alert.alert('Success', 'Livestock posted!', [
            { text: 'OK', onPress: () => router.replace(`/${rolePath}/marketplace/${newId}`) },
          ]);
        }
      }
    } catch (error: any) {
      ...
      <ScrollView className="flex-1 px-4 py-4" keyboardShouldPersistTaps="handled">
        ...
        <View className="mb-4">
          ...
          <TouchableOpacity
            ...
            onPress={() => router.push({
              pathname: `/${rolePath}/marketplace/pick-location`,
              params: {
                _name: name,
                _category: category,
                _price: price,
