...
export default function ChatRoomScreen() {
      ...
      <View className={`flex-row mb-4 ${isCurrentUser ? 'justify-end' : 'justify-start'}`}>
        ...
        >
          ...
          {item.text.trim().length > 0 && (
            <Text className={`text-sm ${isCurrentUser ? 'text-white' : 'text-gray-900 dark:text-gray-100'}`}>
              {item.text}
            </Text>
          )}
          <View className="flex-row items-center justify-end mt-1">
            <Text className={`text-[10px] ${isCurrentUser ? 'text-green-100' : 'text-gray-400'}`}>
  ...
  return (
    ...
    >
      ...
      {selectedImage && (
        <View className="p-2 bg-gray-100 dark:bg-gray-800 flex-row items-center border-t border-gray-200 dark:border-gray-700">
      ...
      <View className="p-3 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 flex-row items-center">
        <TouchableOpacity className="mr-2 p-1" onPress={pickImage}>
          <Ionicons name="camera-outline" size={28} color="#2E7D32" />
        </TouchableOpacity>
        
        <View className="flex-1 bg-gray-100 dark:bg-gray-700 rounded-full px-4 py-2 mr-2 max-h-24">
          <TextInput
