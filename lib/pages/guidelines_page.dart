    ...
    return Column(
      children: [
        ...
        Container(
          ...
          child: TextField(
            decoration: InputDecoration(
              hintText: "Search feeding info...",
              prefixIcon: const Icon(Icons.search, color: AppTheme.primaryColor),
              suffixIcon: _feedingSearchQuery.isNotEmpty
                  ? IconButton(
                      icon: const Icon(Icons.clear),
                      onPressed: () => setState(() => _feedingSearchQuery = ''),
                    )
                  : null,
              filled: true,
              fillColor: AppTheme.backgroundColor,
            ...
            onChanged: (value) {
              setState(() {
                _feedingSearchQuery = value.toLowerCase();
              });
            },
          ),
        ),
        ...
        Expanded(
          child: StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snapshot) {
              ...
              final filteredFeedings = feedings.where((feeding) {
                final feedName = (feeding['name'] ?? '').toString().toLowerCase();
                final category = (feeding['category'] ?? 'Unknown').toString();
                final matchesSearch = _feedingSearchQuery.isEmpty ||
                    feedName.contains(_feedingSearchQuery);
